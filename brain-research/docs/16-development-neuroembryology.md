...
```mermaid
flowchart TD
    A[Neural plate<br/>thickened ectoderm] --> B[Neural groove +<br/>elevating neural folds]
    B --> C[Neural folds fuse<br/>in dorsal midline]
    C --> D[Neural tube<br/>closed]
    C --> E[Neural crest cells<br/>detach dorsally]
    C --> F[Surface ectoderm<br/>re-fuses = epidermis]
    D --> G[Cranial neuropore<br/>closes ~day 24-25]
    D --> H[Caudal neuropore<br/>closes ~day 26-28 at S2]
    H --> I[Below S2: secondary neurulation<br/>from caudal eminence]
    I --> J[Conus medullaris +<br/>filum terminale]
```
...
```mermaid
flowchart TB
    RP["Roof plate — BMP / Wnt (high dorsally)"]
    AP["Alar plate — SENSORY (dorsal horn)"]
    SL["← sulcus limitans →"]
    BP["Basal plate — MOTOR (ventral horn)"]
    FP["Floor plate — Shh source"]
    NC["Notochord — secretes Shh"]
    RP --> AP --> SL --> BP --> FP --> NC
```
...
```mermaid
flowchart LR
    NT[Neural tube] --> P[Prosencephalon]
    NT --> M[Mesencephalon]
    NT --> R[Rhombencephalon]
    P --> T[Telencephalon]
    P --> D[Diencephalon]
    M --> M2[Mesencephalon]
    R --> Met[Metencephalon]
    R --> My[Myelencephalon]
    T --> Tc[Cerebral hemispheres,<br/>cortex, basal ganglia]
    D --> Dc[Thalamus, hypothalamus,<br/>epithalamus, retina/optic n.]
    M2 --> Mc[Midbrain: tectum,<br/>tegmentum, crura]
    Met --> Mtc[Pons + cerebellum]
    My --> Myc[Medulla oblongata]
```
