...
```mermaid
graph TD
    subgraph "Floor of the 4th ventricle (one side)"
      direction LR
      MS[Median sulcus] --- GSE["GSE<br/>somatic motor<br/>III IV VI XII"]
      GSE --- SVE["SVE<br/>branchiomotor<br/>V VII, nuc. ambiguus"]
      SVE --- GVE["GVE<br/>parasympathetic<br/>EW, salivatory, DMNX"]
      GVE --- SL[[Sulcus limitans]]
      SL --- GVA["GVA<br/>visceral afferent<br/>NTS caudal"]
      GVA --- SVA["SVA<br/>taste<br/>NTS rostral"]
      SVA --- GSA["GSA<br/>trigeminal complex"]
      GSA --- SSA["SSA<br/>cochlear + vestibular"]
    end
```
...
```mermaid
graph TD
    subgraph "Rostral medulla — key structures"
      P["Pyramid<br/>(corticospinal)"]
      ML["Medial lemniscus"]
      ION["Inferior olivary nucleus"]
      XII["Hypoglossal nucleus (XII)"]
      DMX["Dorsal motor n. vagus (X)"]
      NTS["Solitary nucleus (taste/visceral)"]
      NA["Nucleus ambiguus (IX,X)"]
      SpV["Spinal trigeminal n. + tract"]
      ICP["Inferior cerebellar peduncle"]
      SYM["Descending sympathetic fibres"]
    end
    P -. "medial territory (ASA)" .- ML
    ML -. medial .- XII
    NA -. "lateral territory (PICA)" .- SpV
    SpV -. lateral .- ICP
    ICP -. lateral .- SYM
```
...
```mermaid
graph TD
    subgraph "Rostral midbrain (superior colliculus level)"
      SC["Superior colliculus (tectum)"]
      AQ["Cerebral aqueduct + PAG"]
      III["Oculomotor nucleus (III) + Edinger–Westphal"]
      RN["Red nucleus"]
      SN["Substantia nigra"]
      CC["Crus cerebri (corticospinal/bulbar)"]
    end
    SC --> AQ --> III --> RN --> SN --> CC
```
...
```mermaid
graph BT
    RF["Brainstem reticular formation<br/>(rostral pons + midbrain)"]
    PPT["PPT/LDT (ACh)"]
    LC["Locus coeruleus (NA)"]
    Raphe["Raphe (5-HT)"]
    Thal["Intralaminar / reticular thalamus"]
    Hypo["Hypothalamus (histamine, orexin)"]
    BF["Basal forebrain (ACh)"]
    Cortex["Cerebral cortex — arousal / wakefulness"]
...
**Rule 3 — There are 4 motor (GSE + GVE) cranial nerve nuclei in the midline**, and their nerve numbers divide evenly into 12: **III, IV** (midbrain), **VI** (pons), **XII** (medulla) — i.e., the ones whose number divides into 12 lie *medially*; the branchiomotor nuclei (V, VII, IX, X) lie *laterally*.

**Rule 4 — There are 4 cranial nerves in the medulla (IX, X, XI, XII), 4 in the pons (V, VI, VII, VIII), and 4 above the pons (in/above the midbrain: III, IV — plus the "sensory" special nerves I, II of the forebrain).** Identifying which cranial nerve is affected therefore pins the *level*; identifying whether the deficit is medial (M) or lateral (S) pins the *side of the tegmentum* and hence the vessel.
