...
```mermaid
flowchart TD
    CN[Any cranial nerve<br/>= a bundle of modalities]
    CN --> EFF[EFFERENT / motor<br/>basal plate, medial]
    CN --> AFF[AFFERENT / sensory<br/>alar plate, lateral]
    EFF --> GSE["GSE — somite muscle<br/>(eye, tongue): III IV VI XII"]
    EFF --> SVE["SVE — arch muscle<br/>(chew, face, pharynx, larynx): V VII IX X XI"]
    EFF --> GVE["GVE — parasympathetic<br/>(glands, gut, heart): III VII IX X"]
    AFF --> GVA["GVA — visceral sensation<br/>(pharynx, carotid, gut): IX X"]
    AFF --> SVA["SVA — smell &amp; taste: I ; VII IX X"]
    AFF --> GSA["GSA — face/head general sensation: V (+VII IX X)"]
    AFF --> SSA["SSA — vision, hearing, balance: II ; VIII"]
```
...
```mermaid
flowchart LR
    EW["Edinger–Westphal<br/>(CN III)"] --> CG[Ciliary ganglion] --> Eye["Pupil + lens<br/>constrict / accommodate"]
    SS["Superior salivatory<br/>(CN VII)"] --> PG[Pterygopalatine ganglion] --> Lac[Lacrimal &amp; nasal glands]
    SS --> SMG[Submandibular ganglion] --> Sal[Submandibular &amp; sublingual glands]
    IS["Inferior salivatory<br/>(CN IX)"] --> OG[Otic ganglion] --> Par[Parotid gland]
    DMV["Dorsal motor nucleus<br/>(CN X)"] --> TG[Terminal ganglia] --> Visc["Heart, lungs, gut<br/>→ splenic flexure"]
```
