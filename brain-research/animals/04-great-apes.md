...
```mermaid
graph LR
    A[More cortical neurons in humans?] --> B[EQ view: brain 3x too big for body]
    A --> C[Scaling view: neuron count normal for a scaled-up primate]
    B --> D[Human brain = anomaly]
    C --> E[Ape bodies unusually large; human cortical neuron count is the extreme, not an exception]
    D --> F[Same data, different framing]
    E --> F
```
...
```mermaid
graph TD
    A[Human-chimp genetic differences relevant to brain] --> B[New genes: e.g. ARHGAP11B - cortical progenitor amplification]
    A --> C[Coding changes: e.g. FOXP2 - motor/learning circuits for speech]
    A --> D[Regulatory changes: HARs / enhancers - when & where neural genes fire]
    B --> E[Bigger, more folded cortex]
    C --> F[Substrate for speech & vocal learning]
    D --> G[Rewired neurodevelopmental timing]
    E --> H[Human brain phenotype]
    F --> H
    G --> H
```
